...
const searchElements = (event, nameElement) => {
  ...
  if (elements.length) {
    infoElement.innerHTML = `<p class="result__number-info"><strong>I found ${elements.length} <strong>${nameElement}<strong>  elements/ <br> W tym dokumencie znalazłem ${elements.length} elementy/ów <strong>${nameElement}</strong></p>`
    showInfo(elements, infoElement)
  } else {
    infoElement.innerHTML = `<p class="result__number-info">I have not found any ${nameElement} elements/W tym dokumencie nie znalazłem elementów <strong>${nameElement}</strong></p>`;
    return;
  }
